...
export function announceHorseCard(iso, tips, opts = {}) {
  ...
  for (const t of tips) {
    ...
    // A multiple is one bet with its legs listed under it, not several tips.
    if (t.kind === 'multiple') {
      lines.push(`${who}<b>${esc(t.label || 'MULTIPLE')}</b>`);
      for (const l of t.legs || []) {
        lines.push(`  ${esc(l.time)} ${esc(l.course)} — <b>${esc(l.horse)}</b> ${esc(l.price)}`);
      }
      if (t.summary) lines.push(`<i>${esc(t.summary)}</i>`);
    } else {
...
/**
 * Ante-post bets get their own post. They aren't part of a day's card — they
 * stand for weeks or months — and burying one in "Selections for Thursday"
 * both misrepresents it and loses it the attention it deserves.
 */
export function announceAntePost(bets, opts = {}) {
  const many = bets.length > 1;
  const lines = [`📌 <b>ANTE-POST${many ? ' BETS' : ''}</b>`, ''];

  for (const b of bets) {
    const who = opts.showTipster && b.tipster ? `${esc(b.tipster)} · ` : '';
    lines.push(`${who}<b>${esc(b.event)}</b>${b.eventDate ? ` · ${esc(b.eventDate)}` : ''}`);
    lines.push(`<b>${esc(b.horse)}</b>`);
    lines.push(
      `${esc(b.price)}${b.book ? ` (${esc(b.book)})` : ''} · ${b.stake}pt `
      + `${b.betType === 'ew' ? `e/w ${esc(b.placeTerms || '1/4')} odds` : 'win'}`
    );
    if (b.writeup) lines.push('', `<i>${esc(String(b.writeup).slice(0, 700))}</i>`);
    lines.push('');
  }

  lines.push(
    `<i>${many ? 'These bets stand' : 'This bet stands'} until the race is run. `
    + `Ante-post rules apply — no refund if your selection doesn't run.</i>`,
    '',
...
export function announceHorseResults(iso, tips, dayPts, running, month) {
  ...
  for (const t of tips) {
    ...
    lines.push(`${mark} <b>${esc(t.horse || t.label || 'Multiple')}</b> — ${esc(t.result).toUpperCase()}`);
  }
