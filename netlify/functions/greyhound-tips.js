...
import { announceTip, announceResult, announceGroupWinner } from './telegram.js';
...
export default async function handler(request) {
  ...
  try {
    ...
    // Shout about a paid-group winner in the free channels.
    // Not stored — the group's record lives in the monthly summaries. This is
    // a marketing post, and the month-to-date figure travels with it.
    if (method === 'PUT' && body.shout) {
      const w = body.shout;
      if (!w.dog || !w.track) return json({ error: 'Dog and track are needed' }, 400);

      const months = await readAllMonths();
      const current = months.length ? months[months.length - 1] : null;
      const monthCtx = current ? {
        label: new Date(`${current.month}-01T12:00:00`)
          .toLocaleDateString('en-GB', { month: 'long' }),
        points: Number(current.points) || 0,
        winners: Number(current.winners) || 0,
        tips: Number(current.tips) || 0,
      } : null;

      const telegram = await announceGroupWinner({
        dog: w.dog, trap: w.trap, track: w.track, time: w.time,
        price: w.price, points: w.points, returned: Number(w.returned) || 0,
      }, monthCtx, body.promo || {});
