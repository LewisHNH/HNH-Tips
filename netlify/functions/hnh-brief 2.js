...
//
// NOT scheduled - hnh-cron.js triggers it. Stays HTTP-reachable.
//
// Test:     /.netlify/functions/hnh-brief?pw=YOUR_ADMIN_PASSWORD
// Preview:  /.netlify/functions/hnh-brief?pw=YOUR_ADMIN_PASSWORD&dry=1
...
exports.handler = async (req) => {
  ...
  // Always requires the password. Called manually by you, or internally
  // by hnh-cron.js at 6:45. No schedule on this file, so it stays
  // reachable over HTTP for testing.
  if (q.pw !== process.env.ADMIN_PASSWORD) {
  ...
  try {
    ...
    // ?dry=1 renders the brief in the browser without sending it.
    if (q.dry === '1') {
      return {
        statusCode: 200,
