...
export default async (req) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const to = url.searchParams.get('to') || ukDate();
      const from = url.searchParams.get('from') || (days
        ? addDays(to, -(Math.min(parseInt(days, 10) || 7, 180) - 1))
        : addDays(to, -6));
      return json({ ok: true, from, to, events: await readRange(from, to) });
    }
