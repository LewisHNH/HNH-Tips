...
// Carried-over monthly totals from the published archive. Aggregates only -
// no per-bet detail exists for these. Must match hnh-webhook.js or the two
// will quote different figures for the same month.
const HORSE_ARCHIVE = [
...
// The archive covers up to and including this date. Per-bet tips on or
// before it are already inside those totals, so counting both double-counts.
...
async function buildBrief() {
  ...
  // Live per-bet tips only from AFTER the archive cutover, so the two
  // halves of the month do not overlap.
  const horsesM = horses.filter((h) => h.date >= monthStart && h.date > ARCHIVE_THROUGH);
  const archiveThisMonth = HORSE_ARCHIVE.find((m) => m.month === month) || null;
  ...
  if (archiveThisMonth) {
    // Two halves of the same month: a carried-over aggregate up to the
    // cutover, and per-bet records after it. Show both plus the total, so
    // the figure can be reconciled against the published archive.
    const combined = round2(archiveThisMonth.points + mHorses.profit);
    L.push(esc(`Horses ${fmt(combined)} total`));
    L.push(esc(`  archive ${fmt(archiveThisMonth.points)} to ${ARCHIVE_THROUGH}, live ${fmt(mHorses.profit)} since`));
  } else {
  ...
  const nags = [];
  // Only chase tips that can still affect a figure. Anything dated on or
  // before the archive cutover is already inside the carried-over monthly
  // total, so its individual result is irrelevant - and some of those old
  // entries aren't even visible in the tips admin to be settled.
  const pendingHorses = horses.filter(
    (h) => h.result === 'pending' && h.date < today && h.date > ARCHIVE_THROUGH
  ).length;
  ...
  if (whop && whop.membersNeedingAction && whop.membersNeedingAction.length) {
    // Members already removed stay cancelled in Whop forever, so without
    // this the brief nags about the same people every morning for a month.
    let dismissed = [];
    try {
      const d = await getStore(EVENTS_STORE).get('dismissed/members', { type: 'json' });
      if (Array.isArray(d)) dismissed = d;
    } catch { /* none yet */ }

    const expired = whop.membersNeedingAction
      .filter((m) => m.action.startsWith('EXPIRED'))
