...
const HORSE_ARCHIVE = [
  ...
  // Calibrated against the published site figure on 28 Aug: the site
  // showed -7.42 for August with +12.70 recorded per-bet since the 19th,
  // so the pre-cutover portion is -20.12. The earlier -24.70 was read off
  // the landing page mid-month and was a live total, not a frozen one.
  { month: '2026-08', points: -20.12, roiPct: null }, // through 18 Aug only
];
...
const PLATFORM_PLAYBOOK = {
  audienceRhythm: {
    ...
    eveningWindow: '18:30-21:30 - results are in and evening meetings are running. Best window for results and reaction content.',
    hnhFixedPoints: 'Greyhound tips go out at 12:00 noon (free and paid). Free horse tips go out at 18:00 for the next day. Promo content should lead into those, not compete with them: a morning preview builds toward the noon dogs, an afternoon post builds toward the 18:00 horses.',
  ...
  platforms: {
    ...
    telegram: {
      ...
      hnhSchedule: 'FIXED AND NON-NEGOTIABLE: greyhound tips (free trap AND paid Exclusive Group) go out at 12:00 noon. Free horse tips go out at 18:00 for the next day. Never suggest moving these or propose a different time for them - build content around them instead.',
...
function horseProfit(tip) {
  ...
  if (!isEW) {
    if (result === 'win') return round2(stake * (dec - 1));
    return round2(-stake); // a place pays nothing on a win-only bet
  }

  const winPart = result === 'win' ? stake * (dec - 1) : -stake;
  const placePart =
    result === 'win' || result === 'placed' ? stake * ((dec - 1) * frac) : -stake;
  return round2(winPart + placePart);
}
 ...
 */
/**
 * The fixture list. Static data, so a long cache is fine - it only changes
 * when Lewis imports a new one.
 */
async function readFixtures() {
  ...
  try {
    ...
    const res = await fetch(
      `${base}/.netlify/functions/hnh-fixtures?pw=${encodeURIComponent(process.env.ADMIN_PASSWORD)}&days=45`,
      { signal: c.signal }
    );
    return await res.json();
  } catch (err) {
    return { error: err.name === 'AbortError' ? 'fixtures timed out' : err.message };
...
async function buildContext() {
  ...
  const [paidMonths, trapFree, horseTips, recentEvents, whop, funnel, memory, fixtures] = await Promise.all([
  ...
  return {
    ...
    marketing: {
      ...
      majorFestivals: upcomingFixtures(today, 90),
      allFixtures: {
        what: 'Every scheduled race meeting for the next 45 days, by date, with course and code (flat or jumps). This is the full fixture list, not just the festivals. If dataPresent is false the list has not been imported yet - say so rather than guessing at a date.',
        ...fixtures,
      },
...
You are not a read-only assistant. You can post messages to Lewis's Telegram channels, and he will always see a preview and confirm before anything sends. Never tell him you cannot send messages or that he will have to post it himself - that is wrong.

SOME ACTIONS TAKE DETAILS. If he asks to settle a bet, void an entry, cancel a scheduled post, or remember something, reply with ONLY this JSON, no prose:

{"action": "settle", "args": {"id": "abc123", "result": "won|lost|void", "sp": "2/1"}}
{"action": "void", "args": {"id": "abc123", "reason": "wrong price"}}
{"action": "unschedule", "args": {"id": "abc123"}}
{"action": "remember", "args": {"text": "the thing to remember"}}
{"action": "track", "args": {"name": "Horse Name", "note": "why it is worth following"}}
{"action": "untrack", "args": {"name": "Horse Name"}}
{"action": "forget", "args": {"id": "3"}}

If he refers to a bet by name rather than id ("settle Ballymac Boom as won"), look in the data for a matching unsettled tip and use its id. If you cannot find one, say so instead of guessing.

If he asks you to send, post, schedule or announce something to a channel, reply with ONLY this JSON, no prose:

{"broadcast": {"channel": "main|horses|greyhounds|exclusive", "message": "the exact text to post", "when": "ISO 8601 UTC datetime, or now"}}

Channel names: main (Main HNH), horses (free horse tips), greyhounds (free Trap of the Day), exclusive (the PAID group). Match whatever he calls it.

Rules for broadcast:
- Post what he actually said. Tidy obvious typos, keep his voice and his emojis. Do not rewrite it into your own phrasing or add a sign-off he did not ask for.
- Times he says are UK local. Convert to UTC. If the time has already gone today, assume tomorrow.
- If he has not said which channel, or it is genuinely ambiguous, do NOT guess - ask which one.
- If the message would breach the compliance rails, say so and offer a fixed version rather than silently changing it.
...
- marketing.allFixtures is the complete race calendar - use it for any question about when or where racing is on, which is the next jumps or flat meeting, or what is running on a given day. marketing.majorFestivals is the shortlist of big meetings worth building content around. Never invent a fixture that is not in either.
- Ground every idea in marketing.majorFestivals. If a fixture is not in that list, you do not know it is happening - say so rather than inventing a date. Never guess a race date; filming for the wrong day wastes his time.
...
async function askClaude(question, context, opts = {}) {
  // Parsing a tip doesn't need the whole business picture, and sending it
  // would waste seconds and tokens on every tip.
  if (opts.skipContext) {
    ...
    const out = (d.content || []).map((b) => (b.type === 'text' ? b.text : '')).filter(Boolean).join('\n');
    return { text: out, meta: { stopReason: d.stop_reason, chars: out.length, outputTokens: null } };
  }

  const maxTokens = opts.maxTokens || 800;
  const analyseNote = opts.analyse
    ? [
        '',
        'ANALYSIS MODE. Lewis has asked for a proper look, so take the space - up to about 350 words.',
        '',
        'Rules for this:',
        '- Lead with the finding, not the method.',
        '- Distinguish signal from noise. Say the sample size and whether it is big enough to mean anything. Three losing bets is not a trend; thirty is.',
        '- Compare like with like. Paid group against free output, horses against greyhounds, this month against last. Never combine the three records.',
        '- Say what the data CANNOT tell him and why - missing SP, aggregate-only months, no per-bet history before a date. A confident answer built on thin data is worse than an honest gap.',
        '- If something looks bad, say it plainly. He can see the numbers anyway and softening them makes the good findings worthless.',
        '- End with the single most useful thing he could do, or the single most useful thing he could start measuring.',
        '- No bullet-point soup. Write in paragraphs like someone who has actually thought about it.',
      ].join('\n')
    : '';

  const draftNote = opts.draft
    ? [
        '',
        'YOU ARE WRITING THE CONTENT ITSELF, not advice about content. Produce something Lewis could film or post in the next ten minutes without thinking.',
        '',
        'Give exactly this, in this order, with nothing else:',
        '1. HOOK - the first line, spoken or on screen. Under 12 words. It has to work in the first two seconds.',
        '2. SCRIPT - what he actually says, written the way people talk, not the way people write. Mark the beats. State the target length in seconds and why.',
        '3. ON-SCREEN TEXT - any captions burnt into the video, if the format needs them.',
        '4. CAPTION - the real caption, ready to paste, written for THAT platform using the caption rules. Not a description of a caption.',
        '5. WHEN - day, time to post, time to film, and one line on why that slot.',
        '',
        'Rules: no bet slips, no profit screenshots, no urgency, odds as analysis never a call to action. Age gate where the format allows. Ground it in a fixture from marketing.upcomingFixtures - if the topic is not in the calendar, say so rather than inventing a date.',
        'Write in Lewis\'s register: plain UK English, direct, reasoning shown, no hype and no American social-media voice.',
      ].join('\n')
    : '';
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `Business figures:\n\`\`\`json\n${JSON.stringify(context)}\n\`\`\`\n\nQuestion: ${question}${draftNote}${analyseNote}${briefNote}`,
      }],
    }),
  });
  const data = await res.json();
 ...
 */
/**
 * Did the model propose a broadcast? If so, stage it and show the preview.
 * Returns true if handled, so the caller doesn't also send the raw JSON.
 */
/**
 * Actions that carry arguments come back as JSON. Rewrite them into the
 * command the existing chain already handles, rather than duplicating
 * logic in two places.
 */
function actionToCommand(answer) {
  ...
  switch (parsed.action) {
    case 'settle':
      if (!a.id || !a.result) return null;
      return `/settle ${a.id} ${a.result}${a.sp ? ` ${a.sp}` : ''}`;
    case 'void':
      if (!a.id) return null;
      return `/void ${a.id}${a.reason ? ` ${a.reason}` : ''}`;
    case 'unschedule':
      return a.id ? `/unschedule ${a.id}` : null;
    case 'remember':
      return a.text ? `/remember ${a.text}` : null;
    case 'track':
      return a.name ? `/track ${a.name}${a.note ? ` - ${a.note}` : ''}` : null;
    case 'untrack':
      return a.name ? `/untrack ${a.name}` : null;
    case 'forget':
      return a.id ? `/forget ${a.id}` : null;
    default:
      return null;
  }
}

async function tryBroadcast(answer, chatId) {
  const trimmed = answer.trim().replace(/^```(?:json)?\n?/, '').replace(/```$/, '').trim();
  if (!trimmed.startsWith('{')) return false;

  let parsed;
  try { parsed = JSON.parse(trimmed); } catch { return false; }
  if (!parsed || !parsed.broadcast) return false;

  const b = parsed.broadcast;
  const channel = resolveChannel(b.channel);
  if (!channel) {
    await tg('sendMessage', {
      chat_id: chatId,
      text: `I don't know a channel called "${b.channel}". Try main, horses, greyhounds or exclusive.`,
    });
    return true;
  }
  if (!channelChatId(channel.key)) {
    await tg('sendMessage', {
      chat_id: chatId,
      text: `${channel.label} isn't configured - ${channel.env} is missing in Netlify.`,
    });
    return true;
  }
  if (!b.message) {
    await tg('sendMessage', { chat_id: chatId, text: "I couldn't work out what to post." });
    return true;
  }

  const sendNow = !b.when || b.when === 'now';
  const whenIso = sendNow ? null : b.when;
  ...
  await getStore(EVENTS_STORE).setJSON(`pending/${id}`, {
    id, kind: 'broadcast', channel: channel.key, message: b.message, when: whenIso,
  });

  const warn = channel.paid ? '\n\n⚠️ <b>This goes to paying members.</b>' : '';

  await tg('sendMessage', {
    chat_id: chatId,
    parse_mode: 'HTML',
    text: `<b>Preview — not sent</b>\n\nTo: <b>${esc(channel.label)}</b>\nWhen: <b>${esc(whenLabel)}</b>${warn}\n\n———\n${esc(b.message)}\n———`,
    reply_markup: {
      inline_keyboard: [[
        { text: sendNow ? '✅ Send now' : '✅ Schedule it', callback_data: `send:${id}` },
...
/**
 * Health check. Touches every dependency and reports what is reachable.
 * Built because two failures yesterday were silent - the only symptom was
...
async function healthCheck() {
  ...
  const time = async (name, fn) => {
    ...
    try {
      ...
      checks.push({ name, ok: false, ms: Date.now() - start, detail: err.message });
    }
  };

  await time('events store', async () => {
    const store = getStore(EVENTS_STORE);
    const listed = await store.list();
    return `${(listed.blobs || []).length} keys`;
  });

  await time('greyhound store', async () => {
    const listed = await getStore(GREY_STORE).list();
  ...
  await time('horse tips store', async () => {
    const days = await getStore(TIPS_STORE).get('days', { type: 'json' });
    if (!days) throw new Error('no days key');
    const dates = Object.keys(days);
    const tips = Object.values(days).reduce((a, d) => a + (d.tips || []).length, 0);
    return `${dates.length} days, ${tips} tips, latest ${dates.sort().pop()}`;
  });

  await time('whop', async () => {
    const w = await readWhop();
    if (w.error) throw new Error(w.error);
    return `${w.activeMembers} active, £${w.mrr} MRR`;
  });

  await time('funnel tracking', async () => {
    const f = await readFunnel();
    if (f.error) throw new Error(f.error);
    if (!f.dataPresent) throw new Error('no data - snippet not installed on the site');
    return `${f.totals.page_view || 0} views in 30d`;
  });
  ...
  await time('transcription', async () => {
    if (!process.env.STT_API_KEY) throw new Error('STT_API_KEY not set');
    return `${process.env.STT_PROVIDER || 'openai'} key present`;
  });
  ...
  // Config that has no runtime check but breaks things when missing.
  const envs = ['BRAIN_BOT_TOKEN', 'BRIEF_CHAT_ID', 'OWNER_TELEGRAM_ID', 'ADMIN_PASSWORD', 'ANTHROPIC_API_KEY'];
  const missing = envs.filter((e) => !process.env[e]);

  return { checks, missing, totalMs: Date.now() - t0 };
}

// ---------------------------------------------------------------------------
// Channels
//
// The brain bot must be an admin with Post Messages in each of these.
// Names are what Lewis says out loud, so aliases matter more than tidiness.
// ---------------------------------------------------------------------------

const CHANNELS = {
  main: {
    env: 'TELEGRAM_MAIN_CHAT_ID',
    label: 'Main HNH',
    paid: false,
    aliases: ['main', 'hnh', 'main channel', 'general', 'everyone'],
  },
  horses: {
    env: 'TELEGRAM_HORSE_CHAT_ID',
    label: 'Horses by HNH (free)',
    paid: false,
    aliases: ['horses', 'horse', 'horses channel', 'free horses', 'racing'],
  },
  greyhounds: {
    env: 'TELEGRAM_GREYHOUND_CHAT_ID',
    label: 'Greyhounds by HNH (free)',
    paid: false,
    aliases: ['greyhounds', 'greyhound', 'dogs', 'free dogs', 'free trap', 'trap of the day'],
  },
  exclusive: {
    env: 'TELEGRAM_EXCLUSIVE_CHAT_ID',
    label: 'Exclusive Greyhound Group (PAID)',
    paid: true,
    aliases: ['exclusive', 'vip', 'paid', 'paid group', 'exclusive group', 'whop group'],
  },
};

function resolveChannel(name) {
  const n = String(name || '').trim().toLowerCase();
  if (CHANNELS[n]) return { key: n, ...CHANNELS[n] };
  for (const [key, c] of Object.entries(CHANNELS)) {
    if (c.aliases.includes(n)) return { key, ...c };
  }
  // Loose match, so "the dogs channel" still lands.
  for (const [key, c] of Object.entries(CHANNELS)) {
    if (c.aliases.some((a) => n.includes(a))) return { key, ...c };
  }
  return null;
}

function channelChatId(key) {
  const c = CHANNELS[key];
  if (!c) return null;
  return process.env[c.env] || null;
}

// ---------------------------------------------------------------------------
// Scheduled messages
//
// Held in one blob rather than one per message, so the scheduler reads
// exactly once every five minutes no matter how much is queued.
// ---------------------------------------------------------------------------
...
async function readQueue() {
  try {
    const v = await getStore(EVENTS_STORE).get(QUEUE_KEY, { type: 'json' });
    return Array.isArray(v) ? v : [];
  } catch { return []; }
}

async function writeQueue(items) {
  await getStore(EVENTS_STORE).setJSON(QUEUE_KEY, items);
}
...
// ---------------------------------------------------------------------------
// Posting tips to the paid group
//
// Tips are currently typed into Telegram by hand, so adding a separate
// logging step would mean entering everything twice - which nobody keeps
// up. Instead this REPLACES the typing: say it once, check the preview,
// confirm. The per-bet record falls out as a byproduct.
//
// Nothing reaches paying members without Lewis seeing it first.
// ---------------------------------------------------------------------------

// Where each kind of tip goes, and what it should say.
const TIP_ROUTES = {
  'greyhound-vip':  { channels: ['exclusive'], paid: true,  header: '🐾 <b>VIP TRAP OF THE DAY</b>' },
  'greyhound-free': { channels: ['greyhounds'], paid: false, header: '🐾 <b>TRAP OF THE DAY</b>' },
  'greyhound-both': { channels: ['exclusive', 'greyhounds'], paid: true, header: null },
  'horse-free':     { channels: ['horses'],     paid: false, header: '🐎 <b>HORSE TIP</b>' },
};

/**
 * A tagged link on every free post. Without this the funnel can see traffic
 * but not which post produced it, which is why every marketing answer so
 * far has been generic mechanics rather than what actually works for HNH.
 */
function taggedLink(medium, campaign) {
  const base = 'https://whop.com/exclusive-greyhound-group/';
...
const HORSE_TEMPLATE = (t, opts = {}) => {
  const L = [];
  L.push(opts.header || '🐎 <b>HORSE TIP</b>');
  L.push('');
  L.push(`<b>${esc(t.time || '')} ${esc(t.course || t.track || '')}</b>`);
  L.push(`<b>${esc(t.horse || t.dog)}</b>`);
  const bet = t.betType && String(t.betType).toLowerCase() === 'ew' ? 'e/w' : 'win';
  L.push(`${esc(t.stake)}pt ${bet} @ <b>${esc(t.odds)}</b>${t.book ? ` (${esc(t.book)})` : ''}`);
  if (t.notes) { L.push(''); L.push(esc(t.notes)); }
  if (opts.link) {
    L.push('');
    L.push(`<a href="${opts.link}">Greyhound VIP group — see what members get</a>`);
  }
  L.push('');
  L.push('<i>18+ | Please gamble responsibly | BeGambleAware.org</i>');
  return L.join('\n');
};

const TIP_TEMPLATE = (t, opts = {}) => {
  const L = [];
  L.push(opts.header || '🐾 <b>VIP TRAP OF THE DAY</b>');
  L.push('');
  L.push(`<b>${esc(t.time || '')} ${esc(t.track || '')}</b>`);
  L.push(`Trap ${esc(t.trap)} — <b>${esc(t.dog)}</b>`);
  L.push(`${esc(t.stake)}pt win @ <b>${esc(t.odds)}</b>${t.book ? ` (${esc(t.book)})` : ''}`);
  if (t.notes) {
    L.push('');
    L.push(esc(t.notes));
  }
  if (opts.link) {
    L.push('');
    L.push(`<a href="${opts.link}">Full card and results — hoovesnhounds</a>`);
  }
  L.push('');
  L.push('<i>18+ | Please gamble responsibly | BeGambleAware.org</i>');
  return L.join('\n');
...
/**
 * Render a tip for one destination. Free posts carry a tagged link so the
 * funnel can attribute a signup back to the post that caused it.
 */
function renderTip(tip, route, channelKey, date) {
  const isFree = !CHANNELS[channelKey].paid;
  const medium = channelKey === 'greyhounds' ? 'free_trap'
    : channelKey === 'horses' ? 'free_horses' : 'vip';
  const opts = {
    header: route.header || (channelKey === 'exclusive'
      ? '🐾 <b>VIP TRAP OF THE DAY</b>'
      : '🐾 <b>TRAP OF THE DAY</b>'),
    link: isFree ? taggedLink(medium, date) : null,
  };
  return tip.sport === 'horse' ? HORSE_TEMPLATE(tip, opts) : TIP_TEMPLATE(tip, opts);
}

async function stagePendingTip(tipWithRoute) {
  const store = getStore(EVENTS_STORE);
  const { routeKey, ...tip } = tipWithRoute;
  const id = 'tip_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 6);
  await store.setJSON(`pending/${id}`, { id, tip, routeKey, at: new Date().toISOString() });
  return id;
}

async function getPendingTip(id) {
  try {
    return await getStore(EVENTS_STORE).get(`pending/${id}`, { type: 'json' });
  } catch { return null; }
}

/**
 * Post to the paid group and record the per-bet detail. Both, or neither -
 * a tip that goes out without being recorded is how the monthly totals
 * drifted away from the individual bets in the first place.
 */
async function publishTip(pending) {
  const route = TIP_ROUTES[pending.routeKey] || TIP_ROUTES['greyhound-vip'];
  const ts = new Date().toISOString();
  ...
  for (const channelKey of route.channels) {
    const chatId = channelChatId(channelKey);
    if (!chatId) { failed.push(`${channelKey} not configured`); continue; }
    try {
      const sent = await tg('sendMessage', {
        chat_id: chatId,
        parse_mode: 'HTML',
        text: renderTip(pending.tip, route, channelKey, date),
        disable_web_page_preview: true,
      });
      if (!sent.ok) throw new Error(sent.description);
      posted.push({ channelKey, messageId: sent.result.message_id });
    } catch (err) {
      failed.push(`${channelKey}: ${err.message}`);
    }
  }

  if (!posted.length) throw new Error(failed.join('; ') || 'nothing posted');

  // One record per destination. The free trap and the VIP group are
  // separate products with separate records, and combining them would
  // make a published figure wrong.
  let day = [];
  try {
    const raw = await store.get(`day/${date}`, { type: 'json' });
    if (Array.isArray(raw)) day = raw;
  } catch { /* first today */ }
  ...
  for (const p of posted) {
    const rec = {
      id: posted.length > 1 ? `${pending.id}_${p.channelKey.slice(0, 3)}` : pending.id,
      ts,
      type: 'tip.posted',
      actor: 'lewis',
      sport: pending.tip.sport === 'horse' ? 'horses' : 'greyhounds',
      payload: {
        group: p.channelKey === 'exclusive' ? 'exclusive' : 'free',
        channel: p.channelKey,
        ...pending.tip,
        result: 'pending',
  ...
  try { await store.delete(`pending/${pending.id}`); } catch { /* fine */ }
  try { await store.delete(CONTEXT_CACHE_KEY); } catch { /* fine */ }

  return { records, posted, failed };
}

/**
 * Settle a posted tip. Appends a settlement rather than editing the
 * original, so the record stays append-only.
 */
async function settleTip(tipId, result, sp) {
  const store = getStore(EVENTS_STORE);
  const today = ukDate();
  const events = await readEventRange(addDays(today, -60), today);
  const posted = events.find(
    (e) => e.type === 'tip.posted' && (e.id === tipId || e.id.endsWith(tipId))
  );
  ...
  const record = {
    ...
    payload: {
      ...posted.payload,
      settles: posted.id,
      advisedPrice: posted.payload.odds,
      sp: sp || null,
      stake: Number(posted.payload.stake) || 0,
      result,
    },
    v: 1,
  };
...
// Intent routing
//
// Slash commands should be a fallback, not the interface. This turns
// "what bets need settling?" into /pending before the command chain runs,
// so every command is reachable in plain English or by voice.
//
// Deliberately a small, cheap call - no business context, tiny output.
// Roughly a second, and only for messages that look like they want an
// action rather than an answer.
// ---------------------------------------------------------------------------

const ROUTABLE = {
  health: 'system status, is everything working, are the connections up',
  pending: 'which bets or tips need settling, what is unsettled, what is awaiting results',
  expired: 'expired or lapsed members, who needs removing from the group, who has stopped paying',
  sp: 'which tips are missing SP or starting price',
  recent: 'what have I logged recently, recent entries',
  memory: 'what do you remember, show me your memory or notes',
  scheduled: 'what is scheduled or queued to post',
  tracker: 'which horses am I tracking, show the tracker or watchlist',
  brief: 'send me the morning brief, run the brief now',
  analyse: 'a deep analysis of what is working, a proper look at performance',
};

// Phrases that clearly want a conversational answer, not an action.
const ANSWER_HINTS = /\b(how did|how are|how much|what is my|what was|why|compare|explain|tell me about|who is|when did)\b/i;

async function routeIntent(text) {
  // Already a command, or clearly a question - don't spend a call on it.
  if (text.startsWith('/')) return null;
  if (text.length > 220) return null;
  if (ANSWER_HINTS.test(text)) return null;

  const menu = Object.entries(ROUTABLE)
    .map(([cmd, desc]) => `${cmd}: ${desc}`)
    .join('\n');

  try {
    const { text: out } = await askClaude(
      `Which single action is this message asking for? Reply with ONLY the action word, or the word "none" if it is a question, a statement, a request to post something, or anything not in the list.\n\n` +
      `Actions:\n${menu}\n\n` +
      `Message: ${text}`,
      null,
      { maxTokens: 12, skipContext: true }
    );
    const word = out.trim().toLowerCase().replace(/[^a-z]/g, '');
    return ROUTABLE[word] ? word : null;
  } catch (err) {
    console.error('intent routing failed:', err);
    return null; // fall through to a normal answer
  }
}

/**
 * Shows the working behind every figure, bet by bet. Built because three
 * numbers looked wrong and guessing at causes wastes more time than
 * printing the arithmetic.
 */
async function auditHorses() {
  const today = ukDate();
  const monthStart = `${today.slice(0, 7)}-01`;
  const tips = await readHorseTips();

  const thisMonth = tips.filter((t) => t.date >= monthStart);
  const live = thisMonth.filter((t) => !t.countedInArchive);
  const inArchive = thisMonth.filter((t) => t.countedInArchive);
  const pending = tips.filter((t) => t.result === 'pending' && t.date > ARCHIVE_THROUGH);
  const archivedPending = tips.filter((t) => t.result === 'pending' && t.date <= ARCHIVE_THROUGH);
  const unknown = tips.filter((t) => t.result === 'unknown');

  return { today, monthStart, tips, thisMonth, live, inArchive, pending, archivedPending, unknown };
}

/**
 * Members already dealt with, so the brief stops nagging about people who
 * have been removed. Whop still shows them cancelled - that is correct -
 * but the job is done.
 */
const DISMISSED_KEY = 'dismissed/members';

async function readDismissed() {
  try {
    const v = await getStore(EVENTS_STORE).get(DISMISSED_KEY, { type: 'json' });
    return Array.isArray(v) ? v : [];
  } catch { return []; }
}

async function dismissMembers(ids) {
  const store = getStore(EVENTS_STORE);
  const current = await readDismissed();
  const merged = [...new Set([...current, ...ids])];
  await store.setJSON(DISMISSED_KEY, merged);
  try { await store.delete(CONTEXT_CACHE_KEY); } catch { /* fine */ }
...
// Horse tracker
//
// Watch a horse, get told when it is next entered. The watchlist works
// without a data feed; the alerting needs one (see hnh-racing.js).
// ---------------------------------------------------------------------------

const TRACKER_KEY = 'tracker/horses';

async function readTracker() {
  try {
    const v = await getStore(EVENTS_STORE).get(TRACKER_KEY, { type: 'json' });
    return Array.isArray(v) ? v : [];
  } catch { return []; }
}

async function trackHorse(name, note) {
  const store = getStore(EVENTS_STORE);
  const list = await readTracker();
  const clean = String(name).trim();
  const key = clean.toLowerCase();

  if (list.some((h) => h.key === key)) return { already: true, name: clean };
  ...
  const entry = {
    ...
    lastSeenRunning: null,
    alerted: [],
  };
  list.push(entry);
  await store.setJSON(TRACKER_KEY, list);
  try { await store.delete(CONTEXT_CACHE_KEY); } catch { /* fine */ }
  return { added: true, entry };
}

async function untrackHorse(name) {
  const store = getStore(EVENTS_STORE);
  const list = await readTracker();
  const key = String(name).trim().toLowerCase();
  const next = list.filter((h) => h.key !== key && !h.key.includes(key));
  if (next.length === list.length) return false;
  await store.setJSON(TRACKER_KEY, next);
  try { await store.delete(CONTEXT_CACHE_KEY); } catch { /* fine */ }
  return true;
}
...
export default async (req) => {
  ...
  // Button presses from the tip preview.
  if (update && update.callback_query) {
    const cb = update.callback_query;
    const fromId = String(cb.from && cb.from.id);
    if (fromId !== String(process.env.OWNER_TELEGRAM_ID)) return ok();
    ...
    try {
      if (action === 'post') {
        const pending = await getPendingTip(id);
        if (!pending) {
          await tg('answerCallbackQuery', { callback_query_id: cb.id, text: 'That preview has expired.' });
          return ok();
        }
        const { records, posted, failed } = await publishTip(pending);
        await tg('answerCallbackQuery', { callback_query_id: cb.id, text: `Posted to ${posted.length}` });

        const where = posted.map((p) => CHANNELS[p.channelKey].label).join(' + ');
        const ids = records.map((r) => `/settle ${r.id.slice(-6)} won`).join('\n');
        const problem = failed.length ? `\n\n<b>Failed:</b> ${esc(failed.join('; '))}` : '';
        ...
        await tg('editMessageText', {
          ...
          parse_mode: 'HTML',
          disable_web_page_preview: true,
          text: `✅ <b>Posted to ${esc(where)}</b>${problem}\n\n<i>Settle with:</i>\n<code>${esc(ids)}</code>`,
        });
      } else if (action === 'send') {
        const pending = await getPendingTip(id);
        if (!pending) {
        ...
        const target = channelChatId(pending.channel);
        if (!target) throw new Error(`${pending.channel} is not configured`);

        if (!pending.when) {
          const sent = await tg('sendMessage', {
            chat_id: target, text: pending.message, disable_web_page_preview: true,
          });
          if (!sent.ok) throw new Error(sent.description);
          await tg('answerCallbackQuery', { callback_query_id: cb.id, text: 'Sent' });
          await tg('editMessageText', {
            chat_id: chatId, message_id: cb.message.message_id, parse_mode: 'HTML',
            text: `✅ <b>Sent to ${esc(CHANNELS[pending.channel].label)}</b>\n\n———\n${esc(pending.message)}\n———`,
          });
        } else {
          await enqueue({
            id: pending.id, channel: pending.channel, message: pending.message,
            when: pending.when, status: 'queued', createdAt: new Date().toISOString(),
          });
          const when = new Intl.DateTimeFormat('en-GB', {
            timeZone: 'Europe/London', weekday: 'short', day: 'numeric', month: 'short',
            hour: '2-digit', minute: '2-digit',
          }).format(new Date(pending.when));
          await tg('answerCallbackQuery', { callback_query_id: cb.id, text: 'Scheduled' });
          await tg('editMessageText', {
            chat_id: chatId, message_id: cb.message.message_id, parse_mode: 'HTML',
            text: `🕒 <b>Scheduled for ${esc(when)}</b>\nTo ${esc(CHANNELS[pending.channel].label)}\n\n———\n${esc(pending.message)}\n———\n\n<i>/unschedule ${esc(pending.id.slice(-6))} to cancel.</i>`,
          });
        }
        try { await getStore(EVENTS_STORE).delete(`pending/${id}`); } catch { /* fine */ }
      } else if (action === 'removeexp') {
        // Removing paying-turned-lapsed members from the group. Ban then
        // immediately unban - a straight ban would stop them ever rejoining,
        // and lapsed members are exactly the people you want back.
        const target = channelChatId('exclusive');
        if (!target) throw new Error('TELEGRAM_EXCLUSIVE_CHAT_ID is not set');

        const whop = await readWhop();
        const expired = (whop.membersNeedingAction || [])
          .filter((m) => String(m.action).startsWith('EXPIRED') && m.telegramLinked);

        const done = [];
        const failed = [];
        for (const m of expired) {
          try {
            const ban = await tg('banChatMember', {
              chat_id: target, user_id: Number(m.telegramId),
            });
            if (!ban.ok) throw new Error(ban.description);
            await tg('unbanChatMember', {
              chat_id: target, user_id: Number(m.telegramId), only_if_banned: true,
            });
            done.push(m.who);
          } catch (err) {
            failed.push(`${m.who}: ${err.message}`);
          }
        }

        await tg('answerCallbackQuery', { callback_query_id: cb.id, text: `Removed ${done.length}` });

        const out = [`<b>Removed ${done.length} from the group</b>`, ''];
        for (const w of done) out.push(`• ${esc(w)}`);
        if (failed.length) {
          out.push('');
          out.push('<b>Failed</b>');
          for (const f of failed) out.push(`• ${esc(f)}`);
        }
        out.push('');
        out.push('<i>Unbanned straight after removal, so they can rejoin if they resubscribe.</i>');

        // Mark them dealt with. Whop will still show them cancelled, which
        // is correct - but the job is done, so stop nagging about it.
        const all = (whop.membersNeedingAction || [])
          .filter((m) => String(m.action).startsWith('EXPIRED'))
          .map((m) => m.who);
        await dismissMembers(all);

        await tg('editMessageText', {
          chat_id: chatId, message_id: cb.message.message_id,
          parse_mode: 'HTML', text: out.join('\n'),
        });
      } else if (action === 'dismissexp') {
        const whop = await readWhop();
        const all = (whop.membersNeedingAction || [])
          .filter((m) => String(m.action).startsWith('EXPIRED'))
          .map((m) => m.who);
        await dismissMembers(all);
        await tg('answerCallbackQuery', { callback_query_id: cb.id, text: 'Marked as done' });
        await tg('editMessageText', {
          chat_id: chatId, message_id: cb.message.message_id,
          text: `Marked ${all.length} as already dealt with. They will stop appearing in the brief.`,
        });
      } else if (action === 'cancelexp') {
        await tg('answerCallbackQuery', { callback_query_id: cb.id, text: 'Left alone' });
        await tg('editMessageText', {
          chat_id: chatId, message_id: cb.message.message_id,
          text: 'Left alone - nobody was removed.',
        });
      ...
      await tg('answerCallbackQuery', { callback_query_id: cb.id, text: err.message.slice(0, 180) });
    }
    return ok();
  }
  ...
  // Plain English into a command, where one fits.
  if (!text.startsWith('/')) {
    const intent = await routeIntent(text);
    if (intent) {
      console.log(`routed "${text.slice(0, 40)}" -> /${intent}`);
      text = `/${intent}`;
    }
  }
  ...
  if (text === '/start' || text === '/help') {
    await tg('sendMessage', {
      ...
      text: [
        ...
        '/health — check every connection',
        '/tip … — post a tip to the Exclusive Group (with preview)',
        '/settle abc123 won 2/1 — record the result',
        '/draft tiktok St Leger — write the actual script and caption',
        '/analyse — a proper look at what is working',
        '/sp — which settled tips are missing an SP',
        '',
        '<b>Broadcasting</b>',
        '/post to horses at 6pm: … — post or schedule to any channel',
        '/scheduled — what is queued',
        '/unschedule abc123 — cancel a queued post',
        '',
        '<b>Housekeeping</b>',
        '/pending — tips still awaiting a result',
        '/expired — lapsed members, with one-tap removal',
        '',
        '<b>Tracker</b>',
        '/track Wild Dahlia - needs further — watch a horse',
        '/tracker — the watchlist',
        '/untrack Wild Dahlia — stop watching',
  ...
  if (text.startsWith('/track')) {
    const raw = text.replace(/^\/track\s*/i, '').trim();
    if (!raw) {
      await tg('sendMessage', {
        chat_id: chatId, parse_mode: 'HTML',
        text: 'Tell me the horse and why:\n\n<code>/track Wild Dahlia - needs further, one to note over 1m2f</code>\n\nI will flag it when it is next entered.',
      });
      return ok();
    }
    const [name, ...rest] = raw.split(/\s*[-–—:]\s*/);
    const result = await trackHorse(name, rest.join(' - '));
    if (result.already) {
      await tg('sendMessage', { chat_id: chatId, text: `${result.name} is already on the tracker.` });
      return ok();
    }
    const list = await readTracker();
    await tg('sendMessage', {
      chat_id: chatId, parse_mode: 'HTML',
      text: `<b>Tracking ${esc(result.entry.name)}</b>${result.entry.note ? `\n<i>${esc(result.entry.note)}</i>` : ''}\n\n${list.length} horse${list.length > 1 ? 's' : ''} on the tracker.`,
    });
    return ok();
  }

  if (text === '/tracker' || text === '/tracked') {
    const list = await readTracker();
    if (!list.length) {
      await tg('sendMessage', { chat_id: chatId, text: 'Nothing tracked yet. /track Horse Name - your note' });
      return ok();
    }
    const L = [`<b>Tracker</b> (${list.length})`, ''];
    for (const h of list) {
      L.push(`<b>${esc(h.name)}</b> <i>added ${esc(h.addedAt)}</i>`);
      if (h.note) L.push(`   ${esc(h.note)}`);
      if (h.lastSeenRunning) L.push(`   <i>last seen entered ${esc(h.lastSeenRunning)}</i>`);
    }
    L.push('');
    L.push('<i>/untrack Horse Name to remove one.</i>');
    await sendSafe(chatId, L.join('\n'), L.join('\n').replace(/<[^>]+>/g, ''));
    return ok();
  }

  if (text.startsWith('/untrack')) {
    const name = text.replace(/^\/untrack\s*/i, '').trim();
    if (!name) {
      await tg('sendMessage', { chat_id: chatId, text: 'Which horse? /tracker to see the list.' });
      return ok();
    }
    const done = await untrackHorse(name);
    await tg('sendMessage', { chat_id: chatId, text: done ? `Removed ${name} from the tracker.` : `${name} isn't on the tracker.` });
    return ok();
  }
  ...
  if (text === '/audit') {
    await tg('sendChatAction', { chat_id: chatId, action: 'typing' });
    try {
      const a = await auditHorses();
      const L = ['<b>Horse audit</b>', ''];

      L.push(`<b>Every tip this month (${a.thisMonth.length})</b>`);
      for (const t of a.thisMonth) {
        const flag = t.countedInArchive ? ' <i>[in archive]</i>' : '';
        const p = t.profitPts === null ? 'unsettled' : `${t.profitPts > 0 ? '+' : ''}${t.profitPts}`;
        L.push(`${esc(t.date)} ${esc(t.horse)} ${esc(t.price)} ${esc(t.betType)} ${t.stake}pt → <b>${esc(t.result)}</b> ${esc(p)}${flag}`);
      }
      L.push('');

      const liveTotal = a.live.reduce((x, t) => x + (t.profitPts || 0), 0);
      L.push(`<b>Live total (after ${esc(ARCHIVE_THROUGH)}):</b> ${round2(liveTotal)} pts from ${a.live.length} tips`);
      L.push(`<b>Excluded as already in archive:</b> ${a.inArchive.length} tips`);
      L.push('');

      if (a.pending.length) {
        L.push(`<b>Counted as unsettled (${a.pending.length})</b>`);
        for (const t of a.pending) {
          L.push(`• ${esc(t.date)} ${esc(t.horse)} — result field is empty or "pending"`);
        }
        L.push('');
      }
      if (a.archivedPending.length) {
        L.push(`<b>Ignored — before the archive cutover (${a.archivedPending.length})</b>`);
        for (const t of a.archivedPending) L.push(`• ${esc(t.date)} ${esc(t.horse)}`);
        L.push('<i>Inside the carried-over total already, so their results cannot change any figure. Not chased.</i>');
        L.push('');
      }
      if (a.unknown.length) {
        L.push(`<b>Unrecognised result strings (${a.unknown.length})</b>`);
        for (const t of a.unknown) L.push(`• ${esc(t.date)} ${esc(t.horse)}`);
        L.push('<i>These are being skipped entirely - tell me what the value is and I will map it.</i>');
      }

      await sendSafe(chatId, L.join('\n'), L.join('\n').replace(/<[^>]+>/g, ''));
    } catch (err) {
      await tg('sendMessage', { chat_id: chatId, text: `Audit failed: ${err.message}` });
    }
    return ok();
  }
  ...
  if (text === '/pending') {
    ...
    try {
      const today = ukDate();
      const [trap, horseTips, events] = await Promise.all([
        readTrapOfTheDay(),
        readHorseTips(),
        readEventRange(addDays(today, -30), today),
      ]);

      // Tips the brain posted itself, so it can settle them directly.
      const posted = events.filter((e) => e.type === 'tip.posted');
      const settledIds = new Set(
        events.filter((e) => e.type === 'tip.settled' && e.payload.settles)
          .map((e) => e.payload.settles)
      );
      const mine = posted.filter((e) => !settledIds.has(e.id));

      // Tips owned by the other admin modules - listed, not touched.
      const trapPending = (trap ? trap.tips : []).filter((t) => t.result === 'pending' && t.date < today);
      // Pre-cutover tips are inside the archive total already, so their
      // results cannot change anything. Chasing them is noise.
      const horsePending = horseTips.filter(
        (t) => t.result === 'pending' && t.date < today && t.date > ARCHIVE_THROUGH
      );

      if (!mine.length && !trapPending.length && !horsePending.length) {
        await tg('sendMessage', { chat_id: chatId, text: 'Nothing outstanding. Everything before today is settled.' });
        return ok();
      }

      const lines = ['<b>Unsettled</b>', ''];

      if (mine.length) {
        lines.push(`<b>Posted through me — ${mine.length}</b>`);
        for (const e of mine.slice(0, 8)) {
          const p = e.payload;
          lines.push(`<code>${esc(e.id.slice(-6))}</code> ${esc(p.dog)} @ ${esc(p.odds)} · ${esc(p.track || '')}`);
        }
        lines.push('<i>Settle with /settle abc123 won 2/1</i>');
        lines.push('');
      }

      if (trapPending.length) {
        lines.push(`<b>Free trap — ${trapPending.length} awaiting a result</b>`);
        for (const t of trapPending.slice(0, 6)) {
          lines.push(`• ${esc(t.date)} ${esc(t.dog)} @ ${esc(t.oddsAdvised)}`);
        }
        lines.push('<i>Settle these in GreyhoundAdmin - that module owns the record.</i>');
        lines.push('');
      }

      if (horsePending.length) {
        lines.push(`<b>Horses — ${horsePending.length} awaiting a result</b>`);
        for (const t of horsePending.slice(0, 8)) {
          lines.push(`• ${esc(t.date)} ${esc(t.horse)} @ ${esc(t.price)} · ${esc(t.course)}`);
        }
        lines.push('<i>Settle these in the tips admin.</i>');
      }

      await sendSafe(chatId, lines.join('\n'), lines.join('\n').replace(/<[^>]+>/g, ''));
    } catch (err) {
      await tg('sendMessage', { chat_id: chatId, text: `Couldn't check: ${err.message}` });
    }
    return ok();
  }
  ...
  if (text === '/expired') {
    ...
    try {
      const whop = await readWhop();
      if (whop.error) throw new Error(whop.error);

      const dismissed = await readDismissed();
      const expired = (whop.membersNeedingAction || [])
        .filter((m) => String(m.action).startsWith('EXPIRED'))
        .filter((m) => !dismissed.includes(m.who));

      if (!expired.length) {
        await tg('sendMessage', { chat_id: chatId, text: 'Nobody expired. Everyone in the group is paying.' });
        return ok();
      }

      const removable = expired.filter((m) => m.telegramLinked);
      const manual = expired.filter((m) => !m.telegramLinked);

      const lines = [`<b>${expired.length} expired</b>`, ''];
      for (const m of expired) {
        lines.push(`• ${esc(m.who)}${m.telegramLinked ? '' : ' <i>(no Telegram linked)</i>'}`);
        lines.push(`   <i>access ended ${esc(m.accessEnds || 'unknown')}</i>`);
      }
      if (manual.length) {
        lines.push('');
        lines.push(`<i>${manual.length} have no Telegram ID on file, so you will need to find them by name in the group.</i>`);
      }

      const buttons = [];
      if (removable.length) {
        buttons.push({ text: `🚪 Remove ${removable.length}`, callback_data: 'removeexp' });
      }
      buttons.push({ text: '✓ Already done', callback_data: 'dismissexp' });
      buttons.push({ text: '✖️ Leave it', callback_data: 'cancelexp' });
  ...
  if (text.startsWith('/post')) {
    ...
    if (!raw) {
      const list = Object.entries(CHANNELS)
        .map(([k, c]) => `<code>${k}</code> — ${esc(c.label)}${c.paid ? ' 💷' : ''}`)
        .join('\n');
      await tg('sendMessage', {
        chat_id: chatId, parse_mode: 'HTML',
        text: `Tell me what to post, where, and when:\n\n<code>/post to horses at 6pm: Tomorrow's tips are in, 3 selections</code>\n<code>/post to main now: Results are up on the site</code>\n\n<b>Channels</b>\n${list}\n\nYou will always see a preview first.`,
      });
      return ok();
    }
    ...
    try {
      ...
      const { text: parsed } = await askClaude(
        `Parse this posting instruction into JSON. Reply with ONLY the JSON object.\n\n` +
        `Fields:\n` +
        `- channel: one of main, horses, greyhounds, exclusive\n` +
        `- message: the exact text to post, cleaned up but not rewritten in a different voice\n` +
        `- when: ISO 8601 datetime in UTC, or the string "now"\n\n` +
        `Current UTC time is ${nowIso}. UK local time is currently ${new Intl.DateTimeFormat('en-GB', { timeZone: 'Europe/London', hour: '2-digit', minute: '2-digit' }).format(new Date())}. ` +
        `Times spoken are UK local - convert them to UTC. If a time has already passed today, assume tomorrow.\n\n` +
        `Instruction: ${raw}`,
        null,
        { maxTokens: 500, skipContext: true }
      );

      const clean = parsed.trim().replace(/^```(?:json)?\n?/, '').replace(/```$/, '').trim();
      const plan = JSON.parse(clean);

      const channel = resolveChannel(plan.channel);
      if (!channel) {
        await tg('sendMessage', { chat_id: chatId, text: `I don't know a channel called "${plan.channel}". Try main, horses, greyhounds or exclusive.` });
        return ok();
      }
      if (!channelChatId(channel.key)) {
        await tg('sendMessage', { chat_id: chatId, text: `${channel.label} isn't configured - ${channel.env} is missing in Netlify.` });
        return ok();
      }
      if (!plan.message) {
        await tg('sendMessage', { chat_id: chatId, text: "I couldn't work out what to post. Say the message after a colon." });
        return ok();
      }

      const sendNow = !plan.when || plan.when === 'now';
      const whenIso = sendNow ? null : plan.when;
      const whenLabel = sendNow
        ? 'immediately'
        : new Intl.DateTimeFormat('en-GB', {
      ...
      const id = 'msg_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 6);
      await getStore(EVENTS_STORE).setJSON(`pending/${id}`, {
        id, kind: 'broadcast', channel: channel.key, message: plan.message, when: whenIso,
      });
      ...
      console.error('post parse failed:', err);
      await tg('sendMessage', { chat_id: chatId, text: `Couldn't read that: ${err.message}` });
    }
    return ok();
  }
  ...
  if (text === '/scheduled') {
    const q = await readQueue();
    const upcoming = q.filter((i) => i.status === 'queued').sort((a, b) => a.when.localeCompare(b.when));
    if (!upcoming.length) {
      await tg('sendMessage', { chat_id: chatId, text: 'Nothing scheduled.' });
      return ok();
    }
    const lines = ['<b>Scheduled</b>', ''];
    for (const i of upcoming) {
      const when = new Intl.DateTimeFormat('en-GB', {
        timeZone: 'Europe/London', weekday: 'short', day: 'numeric', month: 'short',
        hour: '2-digit', minute: '2-digit',
      }).format(new Date(i.when));
      const c = CHANNELS[i.channel];
      lines.push(`<code>${esc(i.id.slice(-6))}</code> · ${esc(when)} · ${esc(c ? c.label : i.channel)}`);
      lines.push(`   ${esc(i.message.slice(0, 70))}${i.message.length > 70 ? '…' : ''}`);
    }
    lines.push('');
    lines.push('<i>/unschedule abc123 to cancel one.</i>');
    await sendSafe(chatId, lines.join('\n'), lines.join('\n').replace(/<[^>]+>/g, ''));
    return ok();
  }

  if (text.startsWith('/unschedule')) {
    const id = text.replace(/^\/unschedule\s*/i, '').trim();
    const q = await readQueue();
    const target = q.find((i) => i.status === 'queued' && (i.id === id || i.id.endsWith(id)));
    if (!target) {
      await tg('sendMessage', { chat_id: chatId, text: `Nothing queued matching ${esc(id)}.` });
      return ok();
    }
    target.status = 'cancelled';
    await writeQueue(q);
    await tg('sendMessage', { chat_id: chatId, text: 'Cancelled - it will not send.' });
    return ok();
  }

  if (text === '/sp' || text === '/missingsp') {
    await tg('sendChatAction', { chat_id: chatId, action: 'typing' });
    try {
      const trap = await readTrapOfTheDay();
      const horseTips = await readHorseTips();

      const trapMissing = (trap ? trap.tips : [])
        .filter((t) => t.result !== 'pending' && !t.oddsSP);
      const horseMissing = horseTips
        .filter((t) => t.result !== 'pending' && !t.sp);

      const lines = ['<b>Missing SP</b>', ''];
      lines.push('<i>Advised vs SP is the strongest claim you have. Without SP it cannot be calculated at all.</i>');
      lines.push('');

      if (!trapMissing.length && !horseMissing.length) {
        lines.push('Nothing missing. Every settled tip has an SP recorded.');
      } else {
        if (trapMissing.length) {
          lines.push(`<b>Free trap — ${trapMissing.length} settled without SP</b>`);
          for (const t of trapMissing.slice(0, 8)) {
            lines.push(`• ${esc(t.date)} ${esc(t.dog)} @ ${esc(t.oddsAdvised)} (${esc(t.result)})`);
          }
          lines.push('<i>Fix these in GreyhoundAdmin - that module owns the record and I should not write to it behind its back.</i>');
          lines.push('');
        }
        if (horseMissing.length) {
          lines.push(`<b>Horses — ${horseMissing.length} settled without SP</b>`);
          for (const t of horseMissing.slice(0, 8)) {
            lines.push(`• ${esc(t.date)} ${esc(t.horse)} @ ${esc(t.price)} (${esc(t.result)})`);
          }
          lines.push('<i>Fix these in the tips admin.</i>');
        }
        lines.push('');
        lines.push('<i>Anything posted through /tip captures SP at /settle, so this only affects the older ones.</i>');
      }
  ...
  if (text.startsWith('/analyse') || text.startsWith('/analyze')) {
    const question = text.replace(/^\/analy[sz]e\s*/i, '').trim()
      || 'What is actually working and what is not? Compare the paid group against the free output, horses against greyhounds, and say where the real strength is.';

    await tg('sendChatAction', { chat_id: chatId, action: 'typing' });
    const started = Date.now();
    try {
      const context = await getContext();
      const { text: answer, meta } = await askClaude(question, context, {
        maxTokens: 1400,
        analyse: true,
      });
      const elapsed = Date.now() - started;
      const debug = process.env.HNH_DEBUG === 'true'
        ? `\n\n<i>${(elapsed / 1000).toFixed(1)}s · ${meta.outputTokens} tok · ${meta.stopReason}</i>`
        : '';
      ...
      await tg('sendMessage', { chat_id: chatId, text: `Couldn't analyse that: ${err.message}` });
    }
    return ok();
  }

  if (text.startsWith('/draft')) {
    const brief = text.replace(/^\/draft\s*/i, '').trim();
    if (!brief) {
      await tg('sendMessage', {
        chat_id: chatId, parse_mode: 'HTML',
        text: 'Tell me what to write:\n\n<code>/draft tiktok St Leger 250th anniversary</code>\n<code>/draft instagram carousel on August results</code>\n\nI will give you the script, the hook, the caption and when to post it.',
      });
      return ok();
    }
    ...
    try {
      const context = await getContext();
      const { text: answer, meta } = await askClaude(brief, context, {
        maxTokens: 1200,
        draft: true,
      });
      ...
      await tg('sendMessage', { chat_id: chatId, text: `Couldn't draft that: ${err.message}` });
    }
    return ok();
  }

  if (text.startsWith('/tip')) {
    const raw = text.replace(/^\/tip\s*/i, '').trim();
    if (!raw) {
      await tg('sendMessage', {
        chat_id: chatId, parse_mode: 'HTML',
        text: 'Say the tip and I will format it:\n\n<code>/tip 19:28 Sunderland trap 4 Ballymac Boom 5/2 2pts Bet365 needs to ping the lids</code>\n\nYou will see a preview before anything goes out.',
      });
      return ok();
    }
    ...
    try {
      const { text: parsed } = await askClaude(
        `Parse this racing tip into JSON. Reply with ONLY the JSON object, no prose.\n\n` +
        `Fields:\n` +
        `- sport: "greyhound" or "horse"\n` +
        `- destination: "vip" (paid group only), "free" (free channel only), or "both". Default to "vip" for greyhounds and "free" for horses unless he says otherwise.\n` +
        `- time (HH:MM), track (greyhounds) or course (horses), trap (number, greyhounds only), dog or horse (the name)\n` +
        `- odds (keep the fraction exactly as given), stake (number of points), betType ("win" or "ew")\n` +
        `- book (bookmaker, optional), notes (the reasoning, optional)\n\n` +
        `Only include fields actually stated. Never invent a price, a stake or a time.\n\n` +
        `Tip: ${raw}`,
        null,
        { maxTokens: 400, skipContext: true }
      );

      const clean = parsed.trim().replace(/^```(?:json)?\n?/, '').replace(/```$/, '').trim();
      const tip = JSON.parse(clean);

      const runner = tip.dog || tip.horse;
      if (!runner || !tip.odds || !tip.stake) {
        await tg('sendMessage', {
          chat_id: chatId,
          text: `I need at least the name, the price and the stake. Got: ${JSON.stringify(tip)}`,
        });
        return ok();
      }

      const sport = tip.sport === 'horse' ? 'horse' : 'greyhound';
      const dest = ['vip', 'free', 'both'].includes(tip.destination)
        ? tip.destination
        : (sport === 'horse' ? 'free' : 'vip');
      const routeKey = sport === 'horse' ? 'horse-free' : `greyhound-${dest}`;
      const route = TIP_ROUTES[routeKey];

      tip.sport = sport;
      const id = await stagePendingTip({ ...tip, routeKey });

      const date = ukDate();
      const previews = route.channels
        .map((c) => `<b>→ ${esc(CHANNELS[c].label)}</b>\n${renderTip(tip, route, c, date)}`)
        .join('\n\n');

      const warn = route.paid ? '\n⚠️ <b>Goes to paying members.</b>' : '';

      await tg('sendMessage', {
        chat_id: chatId,
        parse_mode: 'HTML',
        text: `<b>Preview — not sent yet</b>${warn}\n\n${previews}`,
        disable_web_page_preview: true,
        reply_markup: {
          inline_keyboard: [[
            { text: `✅ Post to ${route.channels.length} channel${route.channels.length > 1 ? 's' : ''}`, callback_data: `post:${id}` },
            { text: '✖️ Cancel', callback_data: `cancel:${id}` },
          ]],
        },
      });
    } catch (err) {
      console.error('tip parse failed:', err);
      await tg('sendMessage', { chat_id: chatId, text: `Couldn't read that tip: ${err.message}` });
    }
    return ok();
  }

  if (text.startsWith('/settle')) {
    const parts = text.replace(/^\/settle\s*/i, '').trim().split(/\s+/);
    const [id, result, sp] = parts;
    if (!id || !result) {
      await tg('sendMessage', {
        chat_id: chatId, parse_mode: 'HTML',
        text: 'Usage: <code>/settle abc123 won 2/1</code>\nResult is won, lost or void. SP optional but worth adding - it is what proves the advised-price edge.',
      });
      return ok();
    }
    const normalised = ['won', 'win'].includes(result.toLowerCase()) ? 'win'
      : ['lost', 'lose'].includes(result.toLowerCase()) ? 'lose'
      : ['void', 'nr'].includes(result.toLowerCase()) ? 'void' : null;
    if (!normalised) {
      await tg('sendMessage', { chat_id: chatId, text: 'Result should be won, lost or void.' });
      return ok();
    }

    const done = await settleTip(id, normalised, sp);
    if (!done) {
      await tg('sendMessage', { chat_id: chatId, text: `No posted tip matching ${id} in the last 60 days.` });
      return ok();
    }
    await tg('sendMessage', {
      chat_id: chatId, parse_mode: 'HTML',
      text: `Settled <b>${esc(done.posted.payload.dog)}</b> as <b>${esc(normalised)}</b>${sp ? ` (SP ${esc(sp)})` : ''}.${sp ? '' : '\n<i>No SP given - the advised-vs-SP gap can\'t be calculated without it.</i>'}`,
    });
    return ok();
  }
  ...
  if (text === '/health') {
    ...
    const lines = ['<b>Health</b>', ''];
    for (const c of h.checks) {
      const mark = c.ok ? '✅' : '❌';
      lines.push(`${mark} ${esc(c.name)} · ${c.ms}ms`);
      lines.push(`   <i>${esc(String(c.detail).slice(0, 90))}</i>`);
    }
    if (h.missing.length) {
      lines.push('');
      lines.push(`<b>Missing config:</b> ${esc(h.missing.join(', '))}`);
    }
    const failed = h.checks.filter((c) => !c.ok).length;
    lines.push('');
    lines.push(failed
      ? `<b>${failed} of ${h.checks.length} failing.</b>`
      : `<b>All ${h.checks.length} healthy.</b> ${h.totalMs}ms total.`);
  ...
  try {
    ...
    // An action with arguments? Turn it into the command and re-run.
    const asCommand = actionToCommand(answer);
    if (asCommand) {
      console.log(`action -> ${asCommand}`);
      await tg('sendMessage', {
        chat_id: chatId,
        parse_mode: 'HTML',
        text: `<i>${esc(asCommand)}</i>`,
      });
      // Replay it through the webhook so the existing handler runs it.
      await fetch(`${process.env.URL}/.netlify/functions/hnh-webhook`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          update_id: Date.now(),
          message: {
            message_id: Date.now(),
            from: { id: Number(process.env.OWNER_TELEGRAM_ID) },
            chat: { id: chatId },
            text: asCommand,
          },
        }),
    ...
    // Was that a broadcast request rather than an answer?
    if (await tryBroadcast(answer, chatId)) {
      await eventStore.setJSON(seenKey, { status: 'done', at: new Date().toISOString() });
      return ok();
    }
