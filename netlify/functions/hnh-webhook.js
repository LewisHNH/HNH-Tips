...
async function askClaude(question, context, opts = {}) {
  ...
  const briefNote = opts.brief
    ? '\n\nIMPORTANT: keep this answer under 180 words. Be decisive and concrete rather than comprehensive - a short usable answer beats a long one that never arrives.'
    : '';
...
export default async (req) => {
  ...
  if (update && update.think) {
    ...
    try {
      ...
      const answer = await askClaude(update.text, context, { maxTokens: 900 });
      return new Response(JSON.stringify({ html: mdToTelegramHtml(answer) }), {
  ...
  try {
    ...
    const answer = await askClaude(text, context, { maxTokens: 400, brief: true });
