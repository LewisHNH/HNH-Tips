...
export default async (req) => {
  ...
  if (update && update.think) {
    ...
    try {
      ...
      const answer = await askClaude(update.text, context);
  ...
  // Hand off to the background function. Claude needs 10-20 seconds to write
  // anything substantial and this function is killed at 10, so the work
  // cannot happen here. Fire and forget - do not await the response, or the
  // handoff itself eats the budget.
  const base = process.env.URL || process.env.DEPLOY_PRIME_URL;
  try {
