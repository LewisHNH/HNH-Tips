...
const TIP_TEMPLATE = (t) => {
  const L = [];
  L.push('🐾 <b>VIP TRAP OF THE DAY</b>');
  L.push('');
...
async function stagePendingTip(tip) {
  const store = getStore(EVENTS_STORE);
  const id = 'tip_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 6);
  await store.setJSON(`pending/${id}`, { id, tip, at: new Date().toISOString() });
  return id;
}
...
async function publishTip(pending) {
  const chatId = process.env.TELEGRAM_EXCLUSIVE_CHAT_ID;
  if (!chatId) throw new Error('TELEGRAM_EXCLUSIVE_CHAT_ID is not set');

  const sent = await tg('sendMessage', {
    chat_id: chatId,
    parse_mode: 'HTML',
    text: TIP_TEMPLATE(pending.tip),
    disable_web_page_preview: true,
  });
  if (!sent.ok) throw new Error(`post failed: ${sent.description}`);
...
export default async (req) => {
  ...
  if (update && update.callback_query) {
    ...
    try {
      if (action === 'post') {
        ...
        const record = await publishTip(pending);
        await tg('answerCallbackQuery', { callback_query_id: cb.id, text: 'Posted' });
        await tg('editMessageText', {
          chat_id: chatId,
          message_id: cb.message.message_id,
          parse_mode: 'HTML',
          text: `✅ <b>Posted to the Exclusive Group</b>\n\n${TIP_TEMPLATE(pending.tip)}\n\n<i>Recorded as ${esc(record.id.slice(-6))}. Settle it with /settle ${esc(record.id.slice(-6))} won</i>`,
        });
      } else if (action === 'send') {
        const pending = await getPendingTip(id);
        if (!pending) {
  ...
  if (text.startsWith('/tip')) {
    ...
    try {
      const { text: parsed } = await askClaude(
        `Parse this greyhound tip into JSON. Reply with ONLY the JSON object, no prose.\n\nFields: time (HH:MM), track, trap (number), dog, odds (keep the fraction as given), stake (number of points), book (bookmaker, optional), notes (the reasoning, optional).\n\nOnly include fields actually stated. Do not invent anything.\n\nTip: ${raw}`,
        { minimal: true },
        { maxTokens: 300, skipContext: true }
      );

      const clean = parsed.trim().replace(/^```(?:json)?\n?/, '').replace(/```$/, '').trim();
      const tip = JSON.parse(clean);

      if (!tip.dog || !tip.odds || !tip.stake) {
        await tg('sendMessage', {
          chat_id: chatId,
          text: `I need at least the dog, the price and the stake. Got: ${JSON.stringify(tip)}`,
        });
        return ok();
      }

      const id = await stagePendingTip(tip);
      await tg('sendMessage', {
        chat_id: chatId,
        parse_mode: 'HTML',
        text: `<b>Preview — not sent yet</b>\n\n${TIP_TEMPLATE(tip)}`,
        reply_markup: {
          inline_keyboard: [[
            { text: '✅ Post to group', callback_data: `post:${id}` },
            { text: '✖️ Cancel', callback_data: `cancel:${id}` },
          ]],
        },
      });
