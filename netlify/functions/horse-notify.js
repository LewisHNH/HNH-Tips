import { announceHorseCard, announceHorseResults, announceAntePost } from './telegram.js';
...
export default async function handler(request) {
  ...
  try {
    ...
    if (type === 'antepost') {
      const telegram = await announceAntePost(tips, {
    ...
    return json({ error: 'type must be card, antepost or results' }, 400);
  } catch (error) {
