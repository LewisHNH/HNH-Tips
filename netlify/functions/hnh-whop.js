...
function normalise(m, plans) {
  ...
  return {
    ...
    affiliate: (() => {
      const u = pick(m, 'affiliate_page_url');
      if (!u) return null;
      const match = String(u).match(/[?&]a=([^&]+)/);
...
function summarise(rows, planFacts = {}) {
  ...
  for (const r of rows) {
    ...
    affiliates[r.affiliate] = (affiliates[r.affiliate] || 0) + 1;
  }
...
export async function getWhopSnapshot({ fresh = false } = {}) {
  ...
  const snapshot = { ...summarise(rows, planFacts), fetchedAt: new Date().toISOString() };
