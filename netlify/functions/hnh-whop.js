...
export default async (req) => {
  ...
  try {
    // Diagnostic: try several endpoints and report exactly what each says.
    // Pins down whether it's the key, the scopes, or the endpoint.
    if (url.searchParams.get('diag') === '1') {
      const key = process.env.WHOP_API_KEY || '';
      const companyId = process.env.WHOP_COMPANY_ID || '';

      const attempts = [
        ['v1 companies (no id needed)', 'https://api.whop.com/api/v1/companies'],
        ['v1 company by id', `https://api.whop.com/api/v1/companies/${companyId}`],
        ['v1 memberships', `https://api.whop.com/api/v1/memberships?company_id=${companyId}&first=1`],
        ['v2 memberships (legacy)', 'https://api.whop.com/api/v2/memberships?per=1'],
        ['v5 company memberships', 'https://api.whop.com/api/v5/company/memberships?per=1'],
      ];
      ...
      for (const [label, target] of attempts) {
        try {
          ...
          results.push({ endpoint: label, error: err.message });
        }
      }

      return json({
        note: 'Trying each endpoint to find which the key actually works against.',
        keyPresent: !!key,
        keyPrefix: key ? key.slice(0, 6) + '...' : null,
        keyLength: key.length,
        companyIdPresent: !!companyId,
        companyIdPrefix: companyId ? companyId.slice(0, 8) + '...' : null,
        companyIdLooksRight: companyId.startsWith('biz_'),
        results,
      });
    }
