...
function PublicView({ days }) {
  ...
  // The live card is the earliest day on or after today that has tips up.
  const liveKey = visible.filter((k) => k >= today).sort()[0] || null;
  /* Previous days are strictly before the live card. Filtering on !== liveKey
     swept tomorrow's card in here too. */
  const past = liveKey ? visible.filter((k) => k < liveKey) : visible;
