...
const PRIOR = {
  ...
  months: [
    ...
    { key: "2026-08", profit: -3.63, bets: 24, winners: 6, placed: 0, staked: 45.5 },
  ],
};
  ...
  return (
    <article className={`tip ${tip.result || "pending"}`}>
      ...
      <div className="body">
        ...
        <div className="meta">
          {tip.oddscheckerUrl ? (
            <a className="price" href={tip.oddscheckerUrl} target="_blank" rel="noopener noreferrer">
              {tip.price} ›
            </a>
...
function PublicView({ days }) {
  ...
  const liveKey = keys.filter((k) => k >= today).sort()[0] || null;
  const past = keys.filter((k) => k !== liveKey);
      ...
      <div className="mail">
        ...
        {sent ? (
          ...
          <div className="mailrow">
            ...
            {/* Point at your MailerLite embed action on the live site */}
            <button className="btn" onClick={() => email.includes("@") && setSent(true)}>Join</button>
          </div>
        )}
      </div>
...
function AdminView({ days, save, exit, password }) {
  ...
  const publish = async () => {
    ...
    try {
      ...
      setFlash(`Published ${clean.length} selections${
        res.telegram?.sent ? " — sent to Telegram." : ". Telegram skipped."}`);
    } catch (e) {
  ...
  const patch = async (iso, id, field, value) => {
    await save({
      ...
      [iso]: {
        ...days[iso],
        tips: days[iso].tips.map((t) => (t.id === id ? { ...t, [field]: value } : t)),
      },
    });
  };
  ...
  const sendResults = async (iso) => {
    setSending(iso);
    try {
      const tips = days[iso].tips;
      const dayPts = tips.reduce((a, t) => a + settlePoints(t), 0);
      const running = summarise(days, "all").profit;
      await notify({
        type: "results", date: iso, dayPts, running,
        tips: tips.map((t) => ({ horse: t.horse, result: t.result })),
      }, password);
      setFlash(`Results for ${shortDate(iso)} sent to Telegram.`);
    } catch (e) {
            ...
            <div key={iso}>
              ...
              {!pendingDays.includes(iso) && (
                <button className="btn flat" style={{ width: "100%" }}
                  ...
                  {sending === iso ? "Sending…" : "Post results to Telegram"}
                </button>
              )}
            </div>
          ))
