...
// True only when every carried-over month has its counts — the board waits for it.
PRIOR.bets = PRIOR.months.reduce((a, m) => a + (m.bets || 0), 0);
PRIOR.winners = PRIOR.months.reduce((a, m) => a + (m.winners || 0), 0);
PRIOR.staked = PRIOR.months.reduce((a, m) => a + (m.staked || 0), 0);
...
function summarise(days, who = "all") {
  ...
  // The carried-over months belong to one tipster only.
  const priorMonths = (who === "all" || who === PRIOR.tipster) ? PRIOR.months : [];
...
function Proof({ s }) {
  ...
  return (
    <>
      ...
      <p className="note" style={{ textAlign: "center" }}>
        {PRIOR.label} carried over from the published archive, where the tips went out
        by Telegram and email. Everything after that was posted here, timestamped.
      </p>
    </>
  );
}
...
function PublicView({ days }) {
  ...
  /* Only days that actually hold tips from whoever is selected. Without this
     the live card is picked globally, so filtering to one tipster can leave
     "Nothing up yet" on screen while their selection sits further down. */
  const visible = keys.filter((k) => tipsFor(k).length);
  /* The live card is the NEWEST day on or after today. Tips go up the evening
     before racing, so from ~6pm today's card and tomorrow's both exist —
     tomorrow's is the one people are coming for, and today's drops below. */
  const liveKey = visible.filter((k) => k >= today).sort().pop() || null;
  // Everything else. Nothing sits after liveKey, so this can't swallow a card.
  const past = visible.filter((k) => k !== liveKey);
...
function AdminView({ days, save, exit, password }) {
  ...
  const [drafts, setDrafts] = useState([blankTip()]);
  const [flash, setFlash] = useState("");
  const [sending, setSending] = useState("");

  /* Drafts always start blank. They used to be pre-filled with whatever was
     already saved for the chosen date, which meant typing a new selection into
     slot one silently overwrote a tip someone else had already posted. */
  useEffect(() => {
    setDrafts([blankTip()]);
    setFlash("");
  }, [date]); // eslint-disable-line

  // Already saved for this date. Shown above the form so it's obvious.
  const alreadyUp = days[date]?.tips || [];

  // Pull a saved tip down into the form to amend it. Keeping its id means the
  // merge on publish updates that tip rather than adding a duplicate.
  const editExisting = (tip) => {
    if (drafts.some((d) => d.id === tip.id)) return;
    setDrafts((d) => [...d.filter((x) => x.horse.trim() || x.course.trim()), { ...tip }]);
    setFlash(`${tip.horse} loaded below — publish to save your changes.`);
  };
  ...
  const publish = async () => {
    ...
    /* Re-read before writing. Two people posting from two devices used to mean
       last-one-to-save wins, and the other tip vanished without warning. */
    let base = days;
    try { base = await store.read(); }
    catch { setFlash("Couldn't re-check the server — publishing from what's loaded."); }

    const before = base[date]?.tips || [];
    const knownIds = new Set(before.map((t) => t.id));
    // Amended tips update in place; genuinely new ones get appended.
    const merged = before
      .map((b) => clean.find((c) => c.id === b.id) || b)
      .concat(clean.filter((c) => !knownIds.has(c.id)));

    await save({
      ...base,
      [date]: { publishedAt: base[date]?.publishedAt || Date.now(), tips: merged },
    });
    setDrafts([blankTip()]);

    /* Only announce tips nobody has seen. Re-publishing to amend a price used
       to re-blast the whole day's card to every channel. */
    const fresh = clean.filter((c) => !knownIds.has(c.id));
    if (!fresh.length) {
      setFlash(`Saved — ${shortDate(date)} updated. Nothing new to send.`);
      return;
    }
    setFlash(`Published — ${fresh.length} new for ${shortDate(date)}.`);
    ...
    try {
      ...
      setFlash(`Published ${fresh.length} selection${fresh.length === 1 ? "" : "s"} — ${tg}, ${ml}.${
  ...
  const sendResults = async (iso, snapshot) => {
    ...
    try {
      ...
      const running = summarise(source, "all").profit;
      const res = await notify({
        type: "results", date: iso, dayPts, running,
    ...
    <>
      ...
      {tab === "post" && (
        <>
          ...
          {alreadyUp.length > 0 && (
            <div className="card" style={{ borderColor: "var(--gold-lo)" }}>
              <h4 style={{ color: "var(--gold)" }}>
                Already up for {shortDate(date)} — {alreadyUp.length} selection{alreadyUp.length === 1 ? "" : "s"}
              </h4>
              <p className="note" style={{ marginTop: 0 }}>
                These stay put. Anything you add below is added alongside them.
              </p>
              {alreadyUp.map((t) => (
                <div key={t.id} style={{
                  ...
                  <span style={{ flex: 1, minWidth: 0, fontSize: 13 }}>
                    ...
                    <small style={{ display: "block", color: "var(--muted)" }}>
                      {tipsterName(t.tipster)} · {t.time} {t.course} · {t.price}
                    </small>
                  </span>
                  <button className="btn flat" onClick={() => editExisting(t)}>Amend</button>
                  <button className="btn flat" onClick={() => removeTip(date, t.id)}>Delete</button>
                </div>
              ))}
            </div>
          )}
              ...
              {days[iso].tips.map((t) => (
                <div className="card" key={t.id}>
                  ...
                  <div className="grid2">
                    ...
                    <div className="field"><label>Override points</label>
                      <input value={t.manualPts} placeholder="Rule 4 / BOG"
                  ...
                  <p className="note">
                    ...
                    {t.betType === "ew" ? " each-way" : " win"}. SP feeds the advised-vs-SP panel.
                  </p>
