...
function AdminView({ days, save, exit, password }) {
  ...
  const patchLeg = async (iso, id, legIdx, field, value) => {
    const updated = days[iso].tips.map((t) => {
      ...
      const next = { ...t, legs };
      return { ...next, result: derivedResult(next) };
    });
  ...
  const publish = async () => {
    const clean = drafts.filter((t) => (isMultiple(t)
      ? (t.legs || []).every((l) => l.horse.trim() && l.price.trim())
      : t.horse.trim() && t.course.trim()));
    if (!clean.length) {
        ...
        recent.length === 0 ? (
          ...
          recent.map((iso) => (
            <div key={iso}>
              ...
              {days[iso].tips.map((t) => (isMultiple(t) ? (
                <div className="card" key={t.id}>
                  <h4>
