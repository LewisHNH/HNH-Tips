...
function AdminView({ days, save, exit, password }) {
  ...
  useEffect(() => {
    // Ante-post bets aren't part of a card, so they don't load into the day's
    // drafts — they're managed from the Settle tab instead.
    if (days[date]) {
      const dayTips = days[date].tips.filter((t) => !isAntePost(t));
      if (dayTips.length) setDrafts(dayTips.map((t) => ({ ...t })));
    }
  ...
  const publish = async () => {
    ...
    /* An ante-post bet is advised today; it isn't part of the card for the
       race day being published. Filing it under that date would date it
       wrongly and drag it into that day's announcements. */
    const dailyDrafts = clean.filter((t) => !isAntePost(t));
    const anteDrafts = clean.filter(isAntePost);
    const anteDay = todayISO();

    /* Drafts are edited copies, so they carry no announcedAt. Merge it back
       from what's saved, or a re-publish would look brand new and re-post a
       card that already went out. */
    const carryStamp = (draft, iso) => {
      const saved = (days[iso]?.tips || []).find((x) => x.id === draft.id);
      return saved?.announcedAt ? { ...draft, announcedAt: saved.announcedAt } : draft;
    };

    const next = { ...days };
    if (dailyDrafts.length) {
      next[date] = {
        publishedAt: days[date]?.publishedAt || Date.now(),
        tips: [
          ...(days[date]?.tips || []).filter(isAntePost),
          ...dailyDrafts.map((t) => carryStamp(t, date)),
        ],
      };
    }
    if (anteDrafts.length) {
      const existing = days[anteDay]?.tips || [];
      const keep = existing.filter((t) => !anteDrafts.some((d) => d.id === t.id));
      next[anteDay] = {
        publishedAt: days[anteDay]?.publishedAt || Date.now(),
        tips: [...keep, ...anteDrafts.map((t) => carryStamp(t, anteDay))],
      };
    }
    await save(next);

    /* The day's card and any ante-post bets go out as separate posts. An
       ante-post bet isn't part of Thursday's card, and putting it there both
       misreads it and loses it in a list of that afternoon's runners. */
    /* Only announce what hasn't been announced. Adding an ante-post bet to a
       day whose card already went out must not re-send that card, and editing
       a write-up shouldn't post the day again either. */
    const isNew = (t) => {
      const saved = (days[isAntePost(t) ? anteDay : date]?.tips || []).find((x) => x.id === t.id);
      return !saved?.announcedAt;
    };
    const daily = dailyDrafts.filter(isNew);
    const ante = anteDrafts.filter(isNew);
    const announced = [];
    const notes = [];

    if (!daily.length && !ante.length) {
      setFlash(`Saved — nothing new to announce (${clean.length} already posted).`);
      return;
    }
    ...
    try {
      if (daily.length) {
        ...
        const e = res.email || {};
        notes.push(t.sent ? `card to ${t.sent}/${t.of} channels` : `card: ${t.skipped || t.error || "not sent"}`);
        notes.push(e.sent ? "email sent" : `email: ${e.skipped || e.error || "not sent"}`);
        if (t.failed) notes.push(`failed: ${t.failed.join("; ")}`);
      }

      if (ante.length) {
        const res = await notify({
          type: "antepost", date, showTipster: TIPSTERS.length > 1,
          tips: ante.map((t) => ({
            horse: t.horse, event: t.event, eventDate: t.eventDate,
            price: t.price, book: t.book, stake: t.stake,
            betType: t.betType, placeTerms: t.placeTerms,
            writeup: t.writeup, tipster: tipsterName(t.tipster),
          })),
        }, password);
        const t = res.telegram || {};
        notes.push(t.sent
          ? `ante-post to ${t.sent}/${t.of} channels`
          : `ante-post: ${t.skipped || t.error || "not sent"}`);
      }

      // Stamp only what was announced, so a later edit can't re-post it.
      announced.push(...daily.map((t) => t.id), ...ante.map((t) => t.id));
      if (announced.length) {
        const stamp = new Date().toISOString();
        const mark = (list) => list.map((t) =>
          (announced.includes(t.id) ? { ...t, announcedAt: stamp } : t));
        const after = { ...next };
        if (next[date]) after[date] = { ...next[date], tips: mark(next[date].tips) };
        if (next[anteDay]) after[anteDay] = { ...next[anteDay], tips: mark(next[anteDay].tips) };
        await save(after);
      }

      setFlash(`Published ${daily.length + ante.length} — ${notes.join(", ")}.`);
    } catch (e) {
