...
const PRIOR_BY_TIPSTER = {
  ...
  nath: {
    ...
    through: "2026-08-18",
    months: [
      { key: "2026-04", profit: 29.40, bets: 29, winners: 15, placed: 0, staked: 45 },
      ...
      { key: "2026-07", profit: 24.10, bets: 33, winners: 11, placed: 0, staked: 48.5 },
      { key: "2026-08", profit: -0.05, bets: 19, winners: 5, placed: 0, staked: 34.25 },
    ],
  },
};

/* Carried-over months for whoever is selected. "All tips" sums both tipsters,
   month by month, so the combined record is the real combined record rather
   than one person's with the other bolted on. */
function priorMonthsFor(who) {
  if (who !== "all") return PRIOR_BY_TIPSTER[who]?.months || [];

  const merged = {};
  for (const { months } of Object.values(PRIOR_BY_TIPSTER)) {
...
const priorLabel = (who) =>
  who === "all"
    ? "April – 18 August 2026"
    : PRIOR_BY_TIPSTER[who]?.label || "";
...
function Proof({ s, who }) {
  ...
  return (
    <>
      ...
      <p className="note" style={{ textAlign: "center" }}>
        {priorLabel(who)} carried over from the published archive, where the tips went out
