...
import { fetchAll, flattenTips, postTip, settleTip, setMembersRow,
         setGroupMonth, setFreeMonth, deleteTip, shoutWinner } from './api.js';
import { WHOP_URL, PROMO_CODE, PROMO_TEXT, SHOUT_PITCH } from './config.js';
...
export default function GreyhoundAdmin({ password = '' }) {
  ...
  async function submit() {
    ...
    setBusy(true);
    try {
      const res = await postTip(form, token);
      setForm({ ...blank(), date: form.date });
      const t = res.telegram || {};
      setMsg(t.sent
        ? `Tip posted — sent to ${t.sent} of ${t.of} channels.${
            t.failed ? ` Failed: ${t.failed.join('; ')}` : ''}`
        : `Tip posted. Telegram: ${t.skipped || t.error || 'not sent'}`);
      await load();
    } catch (err) {
      setMsg(err.message);
    }
  ...
  async function settle(tip, result) {
    ...
    try {
      const oddsSP = result === 'win'
        ...
        : '';
      const res = await settleTip(tip.date, tip.id, result, oddsSP, token);
      const t = res.telegram || {};
      setMsg(t.sent
        ? `Settled — sent to ${t.sent} of ${t.of} channels.${
            t.failed ? ` Failed: ${t.failed.join('; ')}` : ''}`
        : `Settled. Telegram: ${t.skipped || t.error || 'not sent'}`);
      await load();
    } catch (err) {
      setMsg(err.message);
    }
  ...
  // Only offered while the tip is pending and inside the 15 minute window —
  // the server enforces both, this just avoids showing a button that will fail.
  const canRemove = (tip) => {
    if (tip.result && tip.result !== 'pending') return false;
    const age = Date.now() - new Date(tip.postedAt || 0).getTime();
    return Number.isFinite(age) && age <= 15 * 60 * 1000;
  };

  async function remove(tip) {
    if (!window.confirm(`Remove ${tip.dog}? This only works within 15 minutes of posting.`)) return;
    setBusy(true);
    try {
      await deleteTip(tip.date, tip.id, token);
      setMsg(`${tip.dog} removed`);
      await load();
    } catch (err) {
      setMsg(err.message);
    }
  ...
  const [shout, setShout] = useState({
    dog: '', trap: '', track: '', time: '', price: '', points: '', returned: '',
  });

  async function sendShout() {
    if (!shout.dog || !shout.track) {
      setMsg('Dog and track are needed');
      return;
    }
    setBusy(true);
    try {
      const res = await shoutWinner(shout, {
        url: WHOP_URL,
        pitch: SHOUT_PITCH,
        code: PROMO_CODE,
        offer: PROMO_TEXT,
      ...
      const t = res.telegram || {};
      setMsg(t.sent
        ? `Winner posted — sent to ${t.sent} of ${t.of} channels.`
        : `Not sent. ${t.skipped || t.error || ''}`);
      setShout({ dog: '', trap: '', track: '', time: '', price: '', points: '', returned: '' });
    } catch (err) {
      setMsg(err.message);
    }
    setBusy(false);
  ...
  return (
    <div className="gha">
      ...
      <section className="gha-card">
        <h2>Group winner shout</h2>
        <p className="gha-hint">
          Posts a paid-group winner to the free channels, with this month's running
          total attached automatically and the promo code underneath. Nothing is
          stored — the group record stays in the monthly figures above.
        </p>
        <div className="gha-row">
          <label className="gha-field gha-grow"><span>Dog</span>
            <input value={shout.dog} onChange={(e) => setShout({ ...shout, dog: e.target.value })}
                   placeholder="Ballymac Flight" /></label>
          <label className="gha-field gha-narrow"><span>Trap</span>
            <input type="number" inputMode="numeric" min="1" max="6" value={shout.trap}
                   onChange={(e) => setShout({ ...shout, trap: e.target.value })} /></label>
        </div>
        <div className="gha-row">
          <label className="gha-field gha-grow"><span>Track</span>
            <input value={shout.track} onChange={(e) => setShout({ ...shout, track: e.target.value })}
                   placeholder="Nottingham" /></label>
          <label className="gha-field"><span>Time</span>
            <input value={shout.time} onChange={(e) => setShout({ ...shout, time: e.target.value })}
                   placeholder="19:34" /></label>
        </div>
        <div className="gha-row">
          <label className="gha-field"><span>Price</span>
            <input value={shout.price} onChange={(e) => setShout({ ...shout, price: e.target.value })}
                   placeholder="5/2" /></label>
          <label className="gha-field gha-narrow"><span>Points</span>
            <input type="number" inputMode="decimal" step="0.5" value={shout.points}
                   onChange={(e) => setShout({ ...shout, points: e.target.value })} /></label>
          <label className="gha-field gha-narrow"><span>Returned</span>
            <input type="number" inputMode="decimal" step="0.01" value={shout.returned}
                   onChange={(e) => setShout({ ...shout, returned: e.target.value })} /></label>
        </div>
        <button className="gha-primary" onClick={sendShout} disabled={busy}>
          Post winner to Telegram
        </button>
      </section>
      ...
      <section className="gha-card">
        ...
        <p className="gha-hint">
          Settling is final — results can't be edited or deleted afterwards.
          A tip can only be removed within 15 minutes of posting, before it has a result.
        </p>
        ...
        {pending.map((tip) => (
          <div key={tip.id} className="gha-pending">
            ...
            <div className="gha-actions">
              ...
              {canRemove(tip) && (
                <button className="gha-del" onClick={() => remove(tip)} disabled={busy}>Remove</button>
              )}
            </div>
          </div>
        ))}
...
.gha-del{color:rgba(220,110,100,.9)}
.gha-del:hover{color:#E88;border-color:#E88}
`;
