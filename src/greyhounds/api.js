...
export const shoutWinner = (shout, promo, token) => send('PUT', { shout, promo }, token);
export const setFreeMonth = (month, token) => send('PUT', { ...month, kind: 'free' }, token);

/** Only works while a tip is pending and under 15 minutes old. */
export const deleteTip = (date, tipId, token) => send('DELETE', { date, tipId }, token);
